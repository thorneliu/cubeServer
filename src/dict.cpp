...
int Dict::load(const std::string& dict_path, bool in_mem,
...
int Dict::load_index(const std::string& dict_path, const std::string& v_path) {
  ...
  uint32_t file_num = 0;
  if (fread(reinterpret_cast<void*>(&file_num), sizeof(uint32_t), 1,
            pf.get()) != 1) {
    LOG(ERROR) << "index syntax error";
    return E_DATA_ERROR;
  }
  ...
  while (file_cnt > 0) {
    ...
    file_cnt -= to_read;
    if (fread(reinterpret_cast<void*>(&len), sizeof(uint64_t), to_read,
              pf.get()) != to_read) {
      return E_DATA_ERROR;
    }
  ...
  if (_base_dict) {
    if (_base_dict->_slim_table.check_file_len(file_lens, &file_idx)) {
      ...
      file_idx = 0;
      LOG(INFO) << "index check fail, direct copy";
    }
  }
  ...
  for (; file_idx < file_num; ++file_idx) {
    snprintf(file, sizeof(file), "%s%s/index.%lu", dict_path.c_str(),
             v_path.c_str(), file_idx);
...
int Dict::load_data(const std::string& dict_path, const std::string& v_path) {
  ...
  LOG(INFO) << "load data block set count: " << count
            << " , old size: " << old_size;
  ...
  for (size_t i = old_size; i < _block_set.size(); ++i) {
    ...
    snprintf(data_path, 1024, "%s%s/data.%lu", dict_path.c_str(),
             v_path.c_str(), i - old_size);
    ...
    if (fread(reinterpret_cast<void*>(_block_set[i].s_data.get()), sizeof(char),
              _block_set[i].size, data_file) != _block_set[i].size) {
...
int Dict::load_data_mmap(const std::string& dict_path,
  ...
  for (size_t i = old_size; i < _block_set.size(); ++i) {
    ...
    int data_fd = open(data_path, O_RDONLY | O_NONBLOCK,
    ...
    if (block_data && addr + sizeof(uint32_t) <= block_size) {
      ...
      if (addr + len <= block_size && len >= sizeof(uint32_t)) {
        ...
        *buff_size = len - sizeof(uint32_t);
        ...
        memcpy(buff, (block_data + addr + sizeof(uint32_t)),
               len - sizeof(uint32_t));
        return true;
      } else {
        *(reinterpret_cast<uint32_t*>(buff)) = 0;
